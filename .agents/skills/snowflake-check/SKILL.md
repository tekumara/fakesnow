...
- Check `~/.config/fakesnow/.realsnow.toml` for real Snowflake connection info, then the current shell environment for `SNOWFLAKE_*` variables and any repo-local `.env` files.
